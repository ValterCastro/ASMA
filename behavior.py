...
class EmptyGarbage(CyclicBehaviour):
    ...
    async def run(self):
        ...
        fsm.add_state(
            name=STATE_ONE,
            state=StateOne(
                agent=self.agent, central=self.central, trucks=self.available_trucks
            ),
            initial=True,
        )
        ...
        fsm.add_state(
            name=STATE_THREE,
            state=StateThree(
                agent=self.agent, trucks=self.available_trucks, central=self.central
            ),
        )
        fsm.add_state(
            name=STATE_FOUR,
        ...
        fsm.add_state(
            name=STATE_FIVE,
            state=StateFive(agent=self.agent, central=self.central, winner=self.winner),
        )
...
class StateZero(State):
    ...
    async def run(self):
        print(
            f"0️⃣ Bin \033[31m{self.agent.jid}\033[0m is not full - Wait for the bin to be full"
        )
...
class StateOne(State):
    ...
    async def run(self):
        ...
        for key, value in self.central.trucks.items():
            ...
            self.agent.msg_behav = InformBehav(
                agent=self.agent,
...
class StateThree(State):
    ...
    async def run(self):
        print(
            f"3️⃣  Bin \033[31m{self.agent.jid}\033[0m - At least one truck is available"
        )
        ...
        for key, truck in self.trucks.items():
            ...
            truck.msg_behav = InformBehav(
                agent=truck.jid,
                receiver_address=self.agent.jid,
                metadata=data,
                body="I PROPOSE",
            )
...
class StateFour(State):
    ...
    async def run(self):
        ...
        self.agent.msg_behav = InformBehav(
            agent=self.agent,
            receiver_address=str(self.winner.jid),
            metadata=None,
...
class StateFive(State):
    ...
    async def run(self):
        print(f"5️⃣  \033[31m{self.agent.jid}\033[0m - Winner: {self.winner}")
